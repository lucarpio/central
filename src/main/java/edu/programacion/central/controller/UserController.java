...
public class UserController {
    ...
    /*
     * @PostMapping("/usuario/login") public String loginSubmitForm(Model
     * model,@Valid Usuario objUser, HttpServletRequest request, BindingResult
     * result ){
     * 
     * String page=INDEX; model.addAttribute(MODEL_CONTACT, new Usuario());
     * if(result.hasFieldErrors()) { model.addAttribute(MODEL_MESSAGE,
     * "No se ha podido loguear"); }else{ Optional<Usuario> userDB =
     * this.usuariosData.findById(objUser.getUserID()); if(userDB.isPresent()){
     * if(userDB.get().getPassword().equals(objUser.getPassword())){
     * model.addAttribute(MODEL_CONTACT,userDB.get());
     * model.addAttribute(MESSAGE_ATTRIBUTE, "Usuario existe");
     * request.getSession().setAttribute("user", objUser); page="welcome"; }else{
     * model.addAttribute(MESSAGE_ATTRIBUTE, "Password no coincide"); } }else{
     * model.addAttribute(MESSAGE_ATTRIBUTE, "Usuario no existe"); } } return page;
     * }
     */
    /*
